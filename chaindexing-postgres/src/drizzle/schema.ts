...
export const chaindexingEventsSchema = pgTable('chaindexing_events', {
  ...
  logParams: jsonb('log_params').notNull().$type<Record<string, string>>(),
  parameters: jsonb('parameters').notNull().$type<Record<string, string>>(),
  topics: jsonb('topics').notNull().$type<Record<string, string>>(),
